...
const Dashboard = () => {
  ...
  const recentRuns = [
    { id: "run-001", filename: "customers_batch_1.csv", status: "Success", records: 1250, time: "2 hours ago" },
    { id: "run-002", filename: "products_update.csv", status: "In Progress", records: 890, time: "30 min ago" },
    { id: "run-003", filename: "orders_export.csv", status: "Failed", records: 2100, time: "1 hour ago" },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-gray-900 border-gray-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-400">Files Processed (24h)</CardTitle>
            <FileText className="h-4 w-4 text-purple-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-gray-100">{metrics.filesProcessed24h}</div>
        ...
        <Card className="bg-gray-900 border-gray-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-400">Records Migrated</CardTitle>
            <Database className="h-4 w-4 text-blue-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-gray-100">{metrics.recordsMigrated.toLocaleString()}</div>
        ...
        <Card className="bg-gray-900 border-gray-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-100">{metrics.successRate}%</div>
        ...
        <Card className="bg-gray-900 border-gray-800">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-400">Failed Records</CardTitle>
            <AlertTriangle className="h-4 w-4 text-red-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-gray-100">{metrics.failedRecords}</div>
      ...
      <Card className="bg-gray-900 border-gray-800">
        ...
        <CardContent>
          <div className="space-y-4">
            {recentRuns.map((run) => (
              <div key={run.id} className="flex items-center justify-between p-4 bg-gray-800/50 rounded-lg border border-gray-700">
                <div className="flex items-center space-x-4">
                  {getStatusIcon(run.status)}
                  <div>
                    <p className="font-medium text-gray-100">{run.filename}</p>
                    <p className="text-sm text-gray-400">{run.records} records • {run.time}</p>
                  </div>
                </div>
