...
const RunDetail = () => {
  ...
  const runDetails = {
    ...
    failedRecords: [
      {
        row: 345,
        data: { order_id: "ORD-12345", customer_id: "CUST-001", order_date: "2024-13-01", amount: "150.00" },
        error: "Invalid date format: '2024-13-01' is not a valid date"
      },
      {
        row: 1123,
        data: { order_id: "ORD-67890", customer_id: "CUST-999", order_date: "2024-01-15", amount: "75.50" },
        error: "Foreign key constraint violation: customer_id 'CUST-999' does not exist"
      },
      {
        row: 1876,
        data: { order_id: "ORD-11111", customer_id: "CUST-002", order_date: "2024-01-14", amount: "abc" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-100">Migration Run Details</h1>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-100">{runDetails.duration}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-100">{runDetails.recordsProcessed.toLocaleString()}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-400">{runDetails.recordsFailed.toLocaleString()}</p>
              </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-lg font-medium text-gray-100">{runDetails.filename}</p>
            </div>
            <div>
              <p className="text-sm text-gray-400">Success Rate</p>
              <p className="text-lg font-medium text-gray-100">{runDetails.successRate}%</p>
            </div>
            <div>
              <p className="text-sm text-gray-400">Start Time</p>
              <p className="text-lg font-medium text-gray-100">{runDetails.startTime}</p>
            </div>
            <div>
              <p className="text-sm text-gray-400">End Time</p>
              <p className="text-lg font-medium text-gray-100">{runDetails.endTime}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-0">
          <Tabs defaultValue="logs" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-gray-800">
              <TabsTrigger value="logs" className="text-gray-300 data-[state=active]:bg-gray-700">
                Execution Logs
              </TabsTrigger>
              <TabsTrigger value="errors" className="text-gray-300 data-[state=active]:bg-gray-700">
                  ...
                  <Alert key={index} className="bg-red-900/20 border-red-600">
                    ...
                    <AlertDescription>
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-red-300">Row {record.row}</span>
                        ...
                        <div className="text-xs text-gray-400 bg-gray-800 p-2 rounded font-mono">
                          <strong>Data:</strong> {JSON.stringify(record.data, null, 2)}
                        </div>
                      </div>
                    </AlertDescription>
                  </Alert>
